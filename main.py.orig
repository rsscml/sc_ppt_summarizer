...
# SID module imports
from sid_llm_parser import parse_sid_with_llm
from sid_llm_slides import generate_sid_dashboard
from sid_docx_export import sid_spec_to_docx
# summarizer agent
from agent import (
...
async def sid_page():
    return (STATIC_DIR / "sid.html").read_text()
...
# ─── Routes: SID Dashboard ──────────────────────────────────────────

@app.post("/api/sid/upload")
async def upload_sid(file: UploadFile = File(...)):
    """
    Upload a Supplier Impact Tracking Excel file — runs Stages 1 + 2 only.

    Stage 1 — deterministic: Excel → filtered pipe-delimited text table
    Stage 2 — LLM extraction: text table → structured JSON (suppliers)
    ...
    try:
        extracted = await parse_sid_with_llm(
            filepath=str(file_path),
            llm_config=LLM_CONFIG,
            session_id=session_id,
            glossary_context=glossary_prompt_text,
...
async def generate_sid(session_id: str = Form(...), format: str = Form("pptx")):
    """
    Generate the SID dashboard from previously extracted data (Stage 3 + 4).
    Must be called after /api/sid/upload.
    """
    if session_id not in sid_sessions:
        raise HTTPException(status_code=404, detail="SID session not found. Please upload first.")

    sess = sid_sessions[session_id]
    extracted = sess.get("extracted")
    if not extracted:
        raise HTTPException(status_code=400, detail="No extracted data in session.")

    if sess.get("slide_spec") is None:
        pptx_path = str(UPLOAD_DIR / f"{session_id}_sid_dashboard.pptx")
        try:
            buf, slide_spec = await generate_sid_dashboard(
                extracted_data=extracted,
                llm_config=LLM_CONFIG,
                session_id=session_id,
                output_path=pptx_path,
    ...
    return {
        "session_id": session_id,
        "filename": sess["filename"],
        "overall_severity": slide_spec.get("overall_severity", ""),
        "slide_count": slide_spec.get("slide_count", 2),
        "is_fallback": slide_spec.get("_fallback", False),
        "affected_count": len(slide_spec.get("affected_suppliers", [])),
        "detail_count": len(slide_spec.get("supplier_details", [])),
    }


@app.get("/api/sid/download")
async def download_sid_pptx(session_id: str = Query(...)):
    """Download the generated SID dashboard as a .pptx file."""
    if session_id not in sid_sessions:
        raise HTTPException(status_code=404, detail="SID session not found.")

    sess = sid_sessions[session_id]
    if not sess.get("pptx_path"):
        raise HTTPException(status_code=400, detail="Dashboard not yet generated. Call /api/sid/generate first.")
    ...
    source_name = sess.get("filename", "dashboard")
    download_name = f"SID_Dashboard_{source_name.replace('.xlsx', '')}.pptx"
...
@app.get("/api/sid/download/docx")
async def download_sid_docx(session_id: str = Query(...)):
    """Download the SID dashboard as a formatted .docx Word document."""
    if session_id not in sid_sessions:
        raise HTTPException(status_code=404, detail="SID session not found.")
    ...
    buf = sid_spec_to_docx(slide_spec)

    source_name = sess.get("filename", "dashboard")
    download_name = f"SID_Dashboard_{source_name.replace('.xlsx', '')}.docx"
...
@app.get("/api/sid/session/{session_id}")
async def get_sid_session(session_id: str):
    """Return metadata for a SID session."""
    if session_id not in sid_sessions:
        raise HTTPException(status_code=404, detail="SID session not found.")
    ...
    return {
        ...
        "is_fallback": slide_spec.get("_fallback", False),
        "is_generated": sess.get("slide_spec") is not None,
        "supplier_count": len(extracted.get("suppliers", [])),
        "metadata": {
            "total_rows_in_file": meta.get("total_rows_in_file", 0),
            "rows_after_filter": meta.get("rows_after_filter", 0),
            "rows_extracted_by_llm": meta.get("rows_extracted_by_llm", 0),
